...
public Certificate generateForResult(Result result) {
    ...
    public byte[] createPdf(Certificate certificate) {
        try {
            ...
            Font signatureNameFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 14, darkBlueColor);
            ...
            signaturePhrase.add(new Chunk("Sandeep V\n", signatureNameFont));
            signaturePhrase.add(new Chunk("Authorized Signature", footerFont));
            PdfPCell cellSig = new PdfPCell(new Paragraph(signaturePhrase));
